...
class GPTActionTranslator(BaseTranslator):
    ...
    def get_fatal_error(self) -> dict[str, Any] | None:
        with self._fatal_error_lock:
            return None if self._fatal_error is None else dict(self._fatal_error)

    def _record_unrepresentable_request(
        self,
        *,
        mode: str,
        text: str,
        error: QueueItemTooLargeError,
    ) -> None:
        ...
        fatal_error = {
            ...
            "required_chars": error.required_chars,
            "configured_limit": error.max_chars,
        }
        with self._fatal_error_lock:
            if self._fatal_error is None:
                self._fatal_error = fatal_error
        logger.error(
            "GPT Action request cannot be represented: request_id=%s mode=%s "
            "fingerprint=%s required_chars=%s configured_limit=%s",
            error.request_id,
            mode,
            fingerprint,
            error.required_chars,
            error.max_chars,
        )
        ...
        *,
        ...
        try:
            ...
            if not is_batch_request:
                self._record_unrepresentable_request(
                    mode=mode,
                    text=text,
                    error=exc,
                )
